...
# map resolution 지정
const XY_GRID_RESOLUTION = 0.2 #[m]
const YAW_GRID_RESOLUTION = deg2rad(15.0) #[rad]
const GOAL_TYAW_TH = deg2rad(5.0) #[rad]

const MOTION_RESOLUTION = 0.05 #[m]
const N_STEER = 20.0
const EXTEND_AREA = 1.0 #[m]
...
struct Node
    ...
    directions::Array{Int8} # +1 forward, -1 backward # directions of each points forward: true, backward:false
...
mutable struct Path
    ...
    direction::Array{Int8}  # +1 forward, -1 backward # direction forward: true, back false
    cost::Float64 # cost
end

# Collision check sampling helper
@inline function sample_indices(n::Int)
    if n <= 0
        return Int[]
    end
    step = max(1, SKIP_COLLISION_CHECK)
...
function calc_hybrid_astar_path(sx::Float64, sy::Float64, syaw::Float64, syaw1::Float64,
    ...
    syaw1, gyaw1 = rs_path.pi_2_pi(syaw1), rs_path.pi_2_pi(gyaw1)
    println("[INPUT] syaw(rad)=$syaw  syaw(deg)=$(rad2deg(syaw))")
    println("[INPUT] gyaw(rad)=$gyaw  gyaw(deg)=$(rad2deg(gyaw))")
    ...
    # 충돌 체크
    ok_start = trailerlib.check_trailer_collision(ox, oy, [sx], [sy], [syaw], [syaw1], kdtree=kdtree)
    ok_goal = trailerlib.check_trailer_collision(ox, oy, [gx], [gy], [gyaw], [gyaw1], kdtree=kdtree)
    @info "[collision precheck]" ok_start=ok_start ok_goal=ok_goal sx=sx sy=sy gx=gx gy=gy

    c = calc_config(ox, oy, xyreso, yawreso)
    nstart = Node(round(Int64,sx/xyreso), round(Int64,sy/xyreso), round(Int64, syaw/yawreso),true,[sx],[sy],[syaw],[syaw1],Int8[1],0.0,0.0, -1)
    ngoal = Node(round(Int64,gx/xyreso), round(Int64,gy/xyreso), round(Int64,gyaw/yawreso),true,[gx],[gy],[gyaw],[gyaw1],Int8[1],0.0,0.0, -1)
    ...
    while true
        ...
        for i in 1:nmotion
            ...
            steps = MOTION_RESOLUTION * node.directions  # directions: +1/-1
            yaw1  = trailerlib.calc_trailer_yaw_from_xyyaw(node.x, node.y, node.yaw, current.yaw1[end], steps)

            # collision check
            ind = sample_indices(length(node.x))
            if !trailerlib.check_trailer_collision(ox, oy,
                    node.x[ind], node.y[ind], node.yaw[ind], yaw1[ind],
                    kdtree=kdtree)
                bad_coll += 1
                continue
            end
    ...
    path = get_final_path(closed, fnode, nstart, c) # 최종 경로 복원
...
function analystic_expantion(n::Node, ngoal::Node, c::Config, ox, oy, kdtree)
    ...
    while !isempty(pathqueue)
        ...
        ind = sample_indices(length(path.x))
        if trailerlib.check_trailer_collision(ox, oy,
                path.x[ind], path.y[ind], path.yaw[ind], yaw1[ind],
                kdtree = kdtree)
...
function verify_index(node::Node, c::Config, ox, oy, inityaw1, kdtree)
    ...
    ind = sample_indices(length(node.x))
    ok = trailerlib.check_trailer_collision(ox, oy,
            node.x[ind], node.y[ind], node.yaw[ind], yaw1[ind],
            kdtree=kdtree)

    if !ok
        # 버블 내 근처점 개수 대충 찍어서 로그 확인용
...
function calc_next_node(current::Node, c_id::Int64,
    ...
    yaw1list[1] = rs_path.pi_2_pi(current.yaw1[end] + d*MOTION_RESOLUTION/LT*sin(current.yaw[end]-current.yaw1[end])) # 관절각 yaw - yaw1
    ...
    if d > 0
        direction = true
        addedcost += abs(arc_l) # 전진 기본 비용
    else
        direction = false
        addedcost += abs(arc_l) * BACK_COST # 후진 페널티
    end

    # swich back penalty
    if direction != current.direction # switch back penalty
        addedcost += SB_COST # 전/후진 전환 페널티 
    end

    # steer penalyty
    addedcost += STEER_COST*abs(u) # 조향각 크기 페널티

    # steer change penalty
    addedcost += STEER_CHANGE_COST*abs(current.steer - u) # 조향 변화 페널티

    # jacknif cost
    jack = abs.(rs_path.pi_2_pi.(yawlist .- yaw1list))
    addedcost += JACKKNIF_COST * abs(arc_l) * mean(jack) # 잭나이프 페널티

    cost = current.cost + addedcost 

    dir_val::Int8 = d > 0 ? Int8(1) : Int8(-1)
    directions = fill(dir_val, length(xlist))
...
function calc_holonomic_with_obstacle_heuristic(
        gnode::Node, ox::Array{Float64}, oy::Array{Float64}, xyreso::Float64)
    h_dp = grid_a_star.calc_dist_policy(
        gnode.x[end], gnode.y[end],
        ox, oy,
        xyreso,
        HEURISTIC_VR
    )
    inf_cnt = count(isinf, h_dp) # heuristic inf 여부 체크
    tot = length(h_dp)
    @info "[heuristic]" vr=HEURISTIC_VR inf_ratio=(inf_cnt / max(1, tot))
...
function get_final_path(closed::Dict{Int64, Node},
    ...
    direction = Array{Int8}(reverse(ngoal.directions))
    nid = ngoal.pind
...
function calc_cost(n::Node, h_dp::Array{Float64}, ngoal::Node, c::Config)
    ...
    if isinf(h)
        dx = (ngoal.x[end] - n.x[end])
        dy = (ngoal.y[end] - n.y[end])
        h = hypot(dx, dy)
    end

    return n.cost + H_COST * h
end
